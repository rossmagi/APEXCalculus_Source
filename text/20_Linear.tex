...
\mnote{.8}{\textbf{Note:} Though we use $\mu(x)$ for our integrating factor, the symbol is unimportant.  The notation $\mu(x)$ is a common choice, but other texts my use $\alpha(x), I(x)$, or some other symbol to designate the integrating factor.}
