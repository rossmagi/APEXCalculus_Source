...
\example{ex_disease_exponential}{Disease Spread 1}{Suppose a disease spreads at a rate proportional to the number of individuals who are sick.  If 5\% of the population is sick initially and 10\% of the population is sick one week later, find a formula for the proportion of the popoulation that is sick at time $t$.}
...
The conditions $y(0)=0.05$ and $y(1) = 0.1$ lead to $C = 0.05$ a and $k = \ln 2$, so the function is
	\[
		y = 0.05e^{(\ln 2)t}.
	\]
We should point out a glaring problem with this model.  The variable $y$ is a proportion and should be a value between 0 and 1, but the function $y = 0.05e^{2t}$ grows without bound.  After $t \approx 4.32$ weeks, the $y$ exceeds 1, and the model ceases to make physical sense.
}
